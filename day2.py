...
def load_day2_input(filename: str) -> List[int]:
    """
...
def opcode_calc(intcode_lst: List['int']) -> int:
    """
    Replaces the 1st and 2nd index positions
    in the intcode list
    ...
    while True:
        if intcode_lst[current_pos] == 99:
            break
        elif intcode_lst[current_pos] != 1 and intcode_lst[current_pos] != 2:
            break
        elif current_pos >=  lst_len:
            break
        else:
            num1 = current_pos + 1
            num2 = current_pos + 2
            pos_to_change = current_pos + 3
            if intcode_lst[current_pos] == 1:
                intcode_lst[intcode_lst[pos_to_change]] = intcode_lst[intcode_lst[num1]] + intcode_lst[intcode_lst[num2]]
                current_pos += 4
            else:
                intcode_lst[intcode_lst[pos_to_change]] = intcode_lst[intcode_lst[num1]] * intcode_lst[intcode_lst[num2]]
                current_pos += 4
...
def day2(part2: bool = False, input_filename: str = 'input2.txt', num1 = 12, num2 = 2, target=19690720) -> int:
    """
